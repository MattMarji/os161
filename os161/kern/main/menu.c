...
////////////////////////////////////////
/*
 * Command for modifying dbflags -> ON/OFF
 */
static
int
cmd_dbflags(int nargs, char **args)
{
	int dbflags_value_temp; // The reason for a temp is that dbflag_value is the global variable, and we are always adding or subtracting to it.
	int status; //checks if the flag is set to be on or off. where 1 = ON and 0 = OFF
    int temp_nr; // Store the number the user inputs temporarily for use in this command.
	...
	//In this section of the code we must control the status of dbflags_value - a global variable. 
	if (status == 1){
	    dbflags = dbflags | dbflags_value_temp;
        return 0;
    }
    
	else if (status == 0){
        //verify if the bit is already off!
        int temp_dbflags = dbflags;
        kprintf ("dbflags: %d \n", dbflags);
        temp_dbflags = temp_dbflags >>(temp_nr - 1);
        kprintf("Shifted Value %d \n", temp_dbflags);
    
        //check if the value is 0 or 1
        temp_dbflags = temp_dbflags % 2;
        kprintf("Value of temp_dbflags %d \n", temp_dbflags);
        
        if (temp_dbflags == 0){ // already off
            return 0;
        }
        
        else if (temp_dbflags == 1){
            dbflags = dbflags ^ dbflags_value_temp;
            return 0;
            }
    }
...
//Added a [dbflags] section to the options menu for when a user types ?o"
...
//Command to set options for dbflags -> ON/OFF position.
static
int
cmd_debug(int nargs, char **args)
{
	...
	//TEST SOME DEBUG FLAGS HERE!!
	DEBUG(DB_LOCORE, "LOCORE TEST \n");
	DEBUG(DB_SYSCALL, "SYSCALL TEST \n");
	DEBUG(DB_INTERRUPT, "INTERRUPT TEST \n");
	DEBUG(DB_VFS, "VFS TEST \n");
	DEBUG(DB_NETFS, "NETFS TEST \n");
	DEBUG(DB_KMALLOC, "KMALLOC TEST \n");
...
menu(char *args)
{
	...
	menu_execute(args, 1);
	new_thread = *curthread;
	menuthread = &new_thread;
